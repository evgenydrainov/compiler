...
main :: proc() -> i64
{
	...
	if scale(1.5f64, 4.0f64) != 6.0f64 { return 21; }

	// ---- mixed int/float arguments (positional register slots) ----
	// a=2, b=1.5, c=3, d=0.25 -> 1.5*2 + 0.25*3 = 3.75
	if combine(2, 1.5f64, 3, 0.25f64) != 3.75f64 { return 22; }

	// ---- five float args: 5th is passed on the stack ----
	// 0.5 + 0.25 + 0.125 + 0.0625 + 0.0625 = 1.0
	if sum5(0.5f64, 0.25f64, 0.125f64, 0.0625f64, 0.0625f64) != 1.0f64 { return 23; }

	// ---- f32 arguments and return ----
	if scale32(1.5, 4.0) != 6.0 { return 24; }

	// ---- recursion with floats ----
	// 1.0 + 0.5 + 0.25 = 1.75 (powers of two, exact)
	if pow2_sum(3) != 1.75f64 { return 25; }
...
scale32 :: proc(v: f32, factor: f32) -> f32
{
	return v * factor;
}
